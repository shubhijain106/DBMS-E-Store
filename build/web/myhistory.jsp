...
<% 
			...
			Statement st=c.createStatement();
                    ResultSet rs1 = st.executeQuery("select * from history where userht_id='"+ id + "'");
                        
                        	while(rs1.next()){
                                    
                                    uid.add(rs1.getString(1));
                                    itemid.add(rs1.getString(2));
                                    date1.add(rs1.getDate(3));
			...
			e.printStackTrace();
                ...
                <table align="left" border="1" cellspacing="4" cellpadding="3">
                    ...
                    <tr><td><h2>Product Name</h2></td><td colspan="7"><h2>Price</h2></td><td colspan="7"><h2>Date(yyyy-mm-dd)</h2></td><td colspan="7"><h2>Reviews</h2></td></tr>
                      <%       for(int i=0;i<uid.size();i++)
         {
         
         %>  
         <tr><td><h4><%=itemnames.get(i)%></h4></td><td colspan="7"><h2><%=price.get(i)%></h2></td><td colspan="7"><h2><%=date1.get(i)%></h2></td><td colspan="7"><h2><a href=<%="\"random4.jsp?Id="+itemid.get(i)+"/"+id+"/1\""%>>*</a>&nbsp;<a href=<%="\"random4.jsp?Id="+itemid.get(i)+"/"+id+"/2\""%>>*</a>&nbsp;<a href=<%="\"random4.jsp?Id="+itemid.get(i)+"/"+id+"/3\""%>>*</a>&nbsp;<a href=<%="\"random4.jsp?Id="+itemid.get(i)+"/"+id+"/4\""%>>*</a>&nbsp;<a href=<%="\"random4.jsp?Id="+itemid.get(i)+"/"+id+"/5\""%>>*</a></h2></td></tr>
                <%}
                c.close();
                %>
                </table>
