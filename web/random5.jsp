  ...
  <div id="main_content">
    ...
    <div class="serv">
               <% 
                   ...
                   String combine = request.getParameter("Id");
			...
			Statement st=c.createStatement();
                        for(int j=1;j<parts1.length;j++){
                            ResultSet rs=st.executeQuery("select price,item_id from item where item_ID='"+parts1[j]+"'");
                            while(rs.next()){
                        ...
                        int r1=0;
                        ResultSet rs1=null;
                        for( m=0;m<price.size();m++){
                            int r2=st.executeUpdate("update item set quantity_left=quantity_left-1 where item_id='"+name.get(m)+"'");
                         r = st.executeUpdate("insert into history values('"+parts[1]+"','"+name.get(m)+"',(to_date(sysdate, 'dd/mm/yyyy')),'"+price.get(m)+"')");
                       if (r>0)
                        {
                           r1=st.executeUpdate("Delete from cart where userct_id='"+parts[1]+"'");
                        
                        
                        if(r1>0){  
                         
                          rs1=st.executeQuery("select uname from customer where user_id='"+parts[1]+"'");
                          while(rs1.next())
                          {
                                      username=rs1.getString(1);
                                      
                                              }
                          request.getRequestDispatcher("home.jsp?Id="+username).forward(request, response);}
                        }
			else
                        {
                            request.getRequestDispatcher("viewmycart.jsp?Id="+parts[1]).forward(request, response);
                        }
                }}
